...
最終更新: 2026-09-10（**実機ドッグフィード 3 バンドルの一括対応**。診断バンドル `20260910-004226` / `20260910-004934` / `20260910-203646` を解析し、**5 件を修正して 7.23 として完了**（clip\* メニュー全滅・起動時の黒画面・154fps ビジーループ・失敗ダイアログがペインを言わない・失敗 `ReadDirectory` が `active_job_id` を解放しない）。併せて「ジョブを経由しない同期 FS アクセス」を全体監査し **7.21** として起票、うち描画パス内の 2 件（`ui.rs` の `read_dir`・`jump_to_file.rs` の行ごと `is_dir()`）を修正済み。**再発防止の契約テスト `the_render_path_does_not_touch_the_filesystem` を追加**（`rwf-bin/tests/repo_contracts.rs`）。同バンドルの残り 2 項目を **7.22** として起票、クリップメニューの命名整理を **7.20** として設計完了・実装未着手。検証: `cargo fmt --check` / `clippy --all-targets -D warnings` / rwf-lib 1415 / rwf-bin 338+4+8 いずれも全緑）
...
7.7 スマート・トラッシュ・7.11 属性/タイムスタンプ変更・7.12 Create Link/Create File・7.15 デバッグレポート機能・7.17 複数行テキスト入力・7.19 診断スナップショット拡張・7.23 メインスレッド非ブロッキング化 は完了済み）
Phase 6: 全タスク完了（6.1 の colors.json 分離のみ後フェーズ送り）
Phase M: 全タスク完了（詳細: [quality_overhaul.md](quality_overhaul.md) の「Phase M 完了サマリ」）

**🛑 最優先: 作業ツリーの 7.20 が半端に手適用されており、ビルドが通らない（2026-09-10 時点）**
...
- **作業ツリーは未コミット**。7.23 と 7.21 の A1/A2 の実装、`plan/7.20.*.md`・`plan/7.22.*.md` の新規追加、本ファイルの更新がすべてワーキングツリーにある。再開時はまず `git status` と `git diff` で現物を確認すること（設計文書の記述より**コードが一次情報**）。
- **設計は済んで実装が未着手なのは 7.20 のみ**。レビュー済みで残る未決事項は無い（§8 の 3 項目はすべて決着済み）。ただし**リソース JSON だけ手作業で部分適用されており、その状態が壊れている** — 上の 🛑 ブロックを先に読むこと。コード側は一切着手していないので、リソースを 7.20 §2.1 に合わせ直すか `git checkout -- rwf-lib/resources/` で白紙に戻してから、§7 の 4 ピースをピース 1（説明文表示）から順に進めるのが安全。
- 7.22 は設計文書あり・未着手。**先に §3.2（エラー種別の構造化）を潰すこと** — 英語部分文字列マッチのため日本語 Windows では常に "Operation Failed" になる実バグで、タイトルもヒントもこれに依存する。
- 7.21 は残り **カテゴリ B のみ**（Jump to Path の Enter 検証、`get_all_drives()`）。どちらもユーザー操作起点の一発ものなので優先度は低い。

次の作業候補（優先順。実行順の一次情報は上の Phase 7 表の行順）:
1. **7.20 クリップメニューの命名整理** — 設計完了・実装のみ。小規模で独立、再開直後に向く
