...
| 7.7 | **スマート・トラッシュ（ゴミ箱）管理** | `[ ]` | Windows/macOS/Linux 各OS標準への対応。削除ではなくゴミ箱へ移動、復元サポート。**2026-08-05: Task 1-16・19 実装完了**（move/restore/empty/browse-restore UI、確認ダイアログ汎用化、件数/サイズ表示）。**Task 17（自動削除）は 7.13 へスコープアウト**（下記）。残るは Task 18（7.6 側の `InverseJobSpec` 配線確認、7.6 のスコープ）のみ。詳細は [7.7.smart_trash.md](7.7.smart_trash.md) 参照 | ⭐⭐⭐ | 2週間 |
