...
| 7.15 | **デバッグレポート機能（診断セッション）** | `[ ]` | `F12` で診断セッション開始/終了、`F11` で任意の TUI スナップショット採取。1セッション=1フォルダに 構造化イベント(`events.jsonl`)・ログ(`logs.jsonl`)・画面スナップショット・環境メタデータ・ユーザー記述(`report.txt`) を保存し、「操作→状態変化→Job→描画→問題発生」の因果関係を AI が後から再構成できるようにする。**ソース調査で判明した前提修正（2026-08-11）**: `AppEvent` は宣言のみで参照ゼロの死にコード（`app_event.rs`）であり観測点として使えない。実際の choke point は `update_state()`（`state/mod.rs:1044`、`process_pending_events` 経由で JobEvent も合流）・`handle_events()`・`render()` の **3箇所のみ**。スナップショットは `terminal.backend().buffer()` で main loop 所有のまま取得可（`render_dialog_to_string` に前例あり）。ログは tracing の 2枚目 Layer で呼び出し側無改修。Ring buffer は v1 スコープ外。**7.6 と並列開発し main へ先行マージ**（7.6 が自身のデバッグに使えるようにするため）。詳細は [7.15.diagnostic_report.md](7.15.diagnostic_report.md) 参照 | ⭐⭐⭐⭐ | 未見積 |
...
最終更新: 2026-08-11（新規 7.15 デバッグレポート機能（診断セッション）を追加。7.6 の直前に配置し、
...
2. **7.15 デバッグレポート機能** — 1 と並列開発。独立 worktree を `main` から切り、**先に `main` へマージ**してから 7.6 worktree 側で `git merge main` して取り込む（7.6 の非同期トランザクション実装が生むタイミング/状態不整合バグの調査に 7.15 自身を使えるようにするため）。7.6 との衝突は `state/mod.rs`・`app.rs` の追記的な変更のみで小さい
