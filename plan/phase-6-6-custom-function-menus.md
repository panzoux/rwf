...
- **メニューのネスト対応**: menu_xxx.json の `MenuItem.Action` に別の menu ファイル名を
  指定できるようにする。実装方針：
  - `Action` 解決時に `*.json` で終わる場合は menu ファイルとして開く
  - ダイアログスタックに積む（Esc で1段階戻る）
  - 再帰深さ上限を設けてループを防ぐ（例: 最大5段階）
  - `custom_functions.json` の `Menu` インラインでも同様に多段階対応可能
- カスタム関数ダイアログへの「戻る」ナビゲーション（TWF の仕様にないが、ネスト対応時は自然に必要）
