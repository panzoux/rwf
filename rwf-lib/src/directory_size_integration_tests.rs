...
mod tests {
    ...
    fn test_concurrent_directory_size_calculations() {
        ...
        let job1_spec = JobSpec::new(JobKind::CalculateSize {
            location: dir1_location.clone(),
        });
        let job2_spec = JobSpec::new(JobKind::CalculateSize {
            location: dir2_location.clone(),
        });
        let job3_spec = JobSpec::new(JobKind::CalculateSize {
            location: dir3_location.clone(),
        });
    ...
    fn test_size_calculation_updates_multiple_panes() {
        ...
        assert_eq!(
            state.current_tab().left_pane.entries[0].calculated_size,
            Some(calculated_size)
        );
        assert_eq!(
            state.current_tab().right_pane.entries[0].calculated_size,
            Some(calculated_size)
        );
    }
