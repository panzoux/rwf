...
mod tests {
    ...
    async fn test_file_comparison() {
        ...
        assert!(matches!(
            event,
            Some(crate::worker_pool::JobEvent::Started(_))
        ));

        // Verify we received a completed event with comparison result
        let event = event_rx.recv().await;
        match event {
            Some(crate::worker_pool::JobEvent::Completed(
                _,
                SuccessData::ComparisonResult(diff),
            )) => {
                ...
                let has_modification = diff
                    .differences
                    .iter()
                    .any(|chunk| chunk.chunk_type == DiffType::Modified);
        ...
        match event {
            Some(crate::worker_pool::JobEvent::Completed(
                ...
                let all_equal = diff
                    .differences
                    .iter()
                    .all(|chunk| chunk.chunk_type == DiffType::Equal);
    ...
    async fn test_file_split() {
        ...
        let total_size: u64 = part_files.iter().map(|e| e.metadata().unwrap().len()).sum();
    ...
    async fn test_file_join() {
        ...
        assert_eq!(
            joined_content, "aaabbbccc",
            "Joined content should match parts in order"
        );
    }
