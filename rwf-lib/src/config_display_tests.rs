...
#[cfg(test)]
mod tests {
    use crate::config::{AppConfig, ColorScheme, DisplayConfig, TimeFormat};
    ...
    fn test_twf_format_flattened_colors() {
        ...
        assert!(
            config.is_ok(),
            "Failed to deserialize TWF format DisplayConfig: {:?}",
            config.err()
        );
        ...
        assert_eq!(
            config.colors.pane_info_background_color,
            Some("Gray".to_string())
        );
        assert!(!config.show_hidden);
        assert_eq!(config.cjk_width, 2);
    ...
    fn test_json_deserialization_with_old_color_names() {
        ...
        assert!(
            config.is_ok(),
            "Failed to deserialize config with old color names"
        );
    ...
    fn test_json_deserialization_with_new_color_names() {
        ...
        assert!(
            config.is_ok(),
            "Failed to deserialize config with new color names"
        );
        ...
        assert_eq!(
            config.colors.get_inactive_file_pane_cursor_foreground(),
            "Gray"
        );
        assert_eq!(
            config.colors.get_inactive_file_pane_cursor_background(),
            "DarkGray"
        );
    ...
    fn test_json_deserialization_with_mixed_color_names() {
        ...
        assert!(
            config.is_ok(),
            "Failed to deserialize config with mixed color names"
        );
