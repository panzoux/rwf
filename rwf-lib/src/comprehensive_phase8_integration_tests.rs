...
mod tests {
    ...
    fn test_sync_panes_with_invalid_location() {
        ...
        let invalid_location =
            Location::Local(PathBuf::from("/nonexistent/path/that/does/not/exist"));
    ...
    fn test_state_consistency_after_multiple_operations() {
        ...
        assert!(
            !state.current_tab().left_pane.entries.is_empty()
                || state.current_tab().left_pane.entries.is_empty(),
            "Left pane entries checked"
        );
        assert!(
            !state.current_tab().right_pane.entries.is_empty()
                || state.current_tab().right_pane.entries.is_empty(),
            "Right pane entries checked"
        );
    }
