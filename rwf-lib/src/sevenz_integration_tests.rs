...
#[cfg(test)]
mod tests {
    use crate::backend::archive::{ArchiveHandler, SevenZArchiveHandler, MultiFormatArchiveHandler};
    ...
    async fn test_sevenz_create_and_list_root() {
        ...
        assert!(names.contains(&"readme.txt"), "root should contain readme.txt, got: {:?}", names);
        assert!(names.contains(&"docs"), "root should contain docs/, got: {:?}", names);
    ...
    async fn test_sevenz_list_subdirectory() {
        ...
        let names: Vec<&str> = entries.iter().map(|e| e.name.as_str()).collect();
        assert!(names.contains(&"manual.txt"), "docs/ should contain manual.txt, got: {:?}", names);
        assert!(names.contains(&"guide.txt"), "docs/ should contain guide.txt, got: {:?}", names);
    }
    ...
    async fn test_sevenz_extract_all() {
        ...
        assert!(dest_dir.join("readme.txt").exists(), "readme.txt should be extracted");
    ...
    async fn test_multi_format_routes_sevenz() {
        ...
        let cancel = CancellationToken::new();
        let entries = handler.list_entries(&loc, &cancel).await.unwrap();
        assert!(!entries.is_empty(), "MultiFormatArchiveHandler should list 7z entries");
    }
    ...
    async fn test_multi_format_create_sevenz_by_extension() {
        ...
        assert!(dest_7z.metadata().unwrap().len() > 0, "7z file should not be empty");
    }
    ...
    async fn test_multi_format_create_zip_by_extension() {
        ...
        assert!(zip::ZipArchive::new(file).is_ok(), "should be a valid zip archive");
    }
}
