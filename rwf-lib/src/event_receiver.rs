...
pub fn map_job_event_to_transition(event: JobEvent) -> Transition {
    match event {
        ...
        JobEvent::Completed(job_id, success_data) => Transition::CompleteJob {
            job_id,
            result: OpResult::Success(success_data),
        },

        JobEvent::Failed(job_id, error) => Transition::CompleteJob {
            job_id,
...
pub fn process_pending_events<
    B: crate::backend::FilesystemBackend + 'static,
    A: crate::backend::ArchiveHandler + 'static,
>(
