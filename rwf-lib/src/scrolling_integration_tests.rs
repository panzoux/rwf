...
mod tests {
    ...
    fn test_scroll_offset_reset_on_enter_directory() {
        ...
        let mut entries = vec![create_test_entry("subdir", 0, true, &initial_location)];
        entries.extend(
            (0..30).map(|i| {
                create_test_entry(&format!("file{}.txt", i), 100, false, &initial_location)
            }),
        );
        state.current_tab_mut().left_pane.entries = entries;
    ...
    fn test_scroll_triggers_at_top_offset() {
        ...
        let cursor_in_view =
            state.current_tab().left_pane.cursor - state.current_tab().left_pane.scroll_offset;
    ...
    fn test_scroll_triggers_at_bottom_offset() {
        ...
        assert_eq!(state.current_tab().left_pane.cursor, 20);
        assert!(
            state.current_tab().left_pane.scroll_offset > 0,
            "Expected scroll_offset > 0, got {}",
            state.current_tab().left_pane.scroll_offset
        );
    ...
    fn test_cursor_visibility_maintained() {
        ...
        for i in 0..99 {
            // Stop at 99 to avoid going past the end
            let _result = update_state(
                &mut state,
                Transition::CursorMove {
                    pane: ActivePane::Left,
    ...
    fn test_scroll_with_small_file_list() {
        ...
        for i in 0..10 {
            ...
            assert_eq!(
                state.current_tab().left_pane.scroll_offset,
                0,
                "scroll_offset should be 0 for small lists at step {}",
                i
            );
        }
    }
    ...
    fn test_scroll_with_cursor_jump() {
        ...
        assert!(
            cursor_in_view < 20,
            "Cursor not visible after jump to middle"
        );
