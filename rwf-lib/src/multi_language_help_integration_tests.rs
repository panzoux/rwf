...
fn test_language_rotation_cycles_through_all_languages() {
    ...
    for lang in &available_languages {
        assert!(
            seen_languages.contains(lang),
            "Language {} not seen in rotation",
            lang
        );
    }
}
...
fn test_help_dialog_displays_all_key_bindings() {
    ...
    let required_bindings = vec!["Tab", "Copy", "Move", "Delete", "Space", "Mark all"];
...
fn test_help_dialog_shows_language_rotation_key() {
    ...
    assert!(
        formatted.to_lowercase().contains("language")
            || formatted.to_lowercase().contains("rotate")
    );
}
