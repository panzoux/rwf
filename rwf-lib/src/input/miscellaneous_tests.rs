    ...
    match &transitions[0] {
        Transition::ShowDialog { dialog } => {
            assert!(
                ...
                "help dialog title must not be empty"
            );
            assert!(
                matches!(&dialog.content, DialogContent::Help { .. }),
                "Expected Help dialog content"
            );
        }
...
fn test_detects_duplicate_in_normal_mode() {
    ...
    assert!(
        warnings[0].contains("'CursorUp'"),
        "should show overridden action"
    );
    assert!(
        warnings[0].contains("'CursorDown'"),
        "should show winning action"
    );
}
...
fn test_detects_duplicates_in_multiple_modes() {
    ...
    assert!(
        vw.contains("'q'") && vw.contains("'ViewerClose'") && vw.contains("'ViewerScrollDown'")
    );
}
