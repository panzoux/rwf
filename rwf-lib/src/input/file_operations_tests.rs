...
mod tests {
    ...
    fn create_test_state() -> AppState {
        ...
        state.current_tab_mut().left_pane.current_location = Location::Local(PathBuf::from("/test"));
        state.current_tab_mut().right_pane.current_location = Location::Local(PathBuf::from("/dest"));
    ...
    fn test_confirm_delete_dialog_creates_job() {
        ...
        match &result.jobs_to_start[0].kind {
            crate::job::JobKind::Delete { targets } => {
                ...
                assert_eq!(targets[0], Location::Local(PathBuf::from("/test/file1.txt")));
            }
    ...
    async fn test_multiple_file_copy_execution() {
        ...
        assert!(dest_dir.join("file1.txt").exists(), "file1.txt should be copied");
        assert!(dest_dir.join("file2.txt").exists(), "file2.txt should be copied");
        assert!(dest_dir.join("file3.txt").exists(), "file3.txt should be copied");
    ...
    async fn test_copy_with_overwrite() {
        ...
        // Create source file
        let source_file = temp_path.join("source.txt");
        tokio::fs::write(&source_file, b"new content").await.unwrap();
        ...
        let content = tokio::fs::read(&dest_file).await.unwrap();
        assert_eq!(content, b"new content", "File should be overwritten with new content");
    }
    ...
    async fn test_copy_directory_recursive() {
        ...
        tokio::fs::write(source_dir.join("file1.txt"), b"content 1").await.unwrap();
        
        let subdir = source_dir.join("subdir");
        tokio::fs::create_dir(&subdir).await.unwrap();
        tokio::fs::write(subdir.join("file2.txt"), b"content 2").await.unwrap();
        ...
        assert!(copied_dir.join("file1.txt").exists(), "file1.txt should be copied");
        assert!(copied_dir.join("subdir").exists(), "subdir should be copied");
        assert!(copied_dir.join("subdir").join("file2.txt").exists(), "file2.txt should be copied");
        
        // Verify content
        let content1 = tokio::fs::read(copied_dir.join("file1.txt")).await.unwrap();
        assert_eq!(content1, b"content 1");
        
        let content2 = tokio::fs::read(copied_dir.join("subdir").join("file2.txt")).await.unwrap();
        assert_eq!(content2, b"content 2");
    }
    ...
    async fn test_multiple_file_move_execution() {
        ...
        assert!(dest_dir.join("file1.txt").exists(), "file1.txt should be moved");
        assert!(dest_dir.join("file2.txt").exists(), "file2.txt should be moved");
        assert!(dest_dir.join("file3.txt").exists(), "file3.txt should be moved");
    ...
    async fn test_delete_cancellation() {
        ...
        let spec = JobSpec::new(JobKind::Delete { targets: targets.clone() });
    ...
    async fn test_rename_with_conflict() {
        ...
        tokio::fs::write(&new_path, b"existing content").await.unwrap();
        ...
        while let Ok(event) = event_rx.try_recv() {
            if let crate::worker_pool::JobEvent::Failed(_, error) = event {
                ...
                assert!(error.contains("already exists"), "Error should mention file already exists");
            }
        }
        ...
        assert_eq!(old_content, b"old content", "Old file content should be unchanged");
        
        let existing_content = tokio::fs::read(&new_path).await.unwrap();
        assert_eq!(existing_content, b"existing content", "Existing file content should be unchanged");
    }
    ...
    async fn test_rename_with_invalid_characters() {
        ...
        while let Ok(event) = event_rx.try_recv() {
            if let crate::worker_pool::JobEvent::Failed(_, error) = event {
                received_failed = true;
                assert!(error.contains("Invalid characters"), "Error should mention invalid characters");
            }
        }
    ...
    async fn test_rename_directory() {
        ...
        assert!(file_in_new_dir.exists(), "File inside directory should be preserved");
    ...
    async fn test_mkdir_with_conflict() {
        ...
        while let Ok(event) = event_rx.try_recv() {
            if let crate::worker_pool::JobEvent::Failed(_, error) = event {
                received_failed = true;
                assert!(error.contains("already exists"), "Error should mention directory already exists");
            }
        }
    ...
    async fn test_mkdir_nested_path() {
        ...
        assert!(temp_path.join("parent").exists(), "Parent directory should be created");
        assert!(temp_path.join("parent").join("child").exists(), "Child directory should be created");
    }
