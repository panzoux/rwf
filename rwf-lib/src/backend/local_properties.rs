...
fn directory_structure() -> impl Strategy<Value = (TempDir, Vec<(PathBuf, u64)>, u64)> {
    ...
    prop::collection::vec(100u64..1000u64, 1..10).prop_map(|file_sizes| {
...
fn nested_directory_structure() -> impl Strategy<Value = (TempDir, u64)> {
    ...
    (1..5usize, prop::collection::vec(100u64..500u64, 1..5)).prop_map(
        |(num_subdirs, file_sizes)| {
            let temp_dir = TempDir::new().unwrap();
            let temp_path = temp_dir.path();
            let mut total_size = 0u64;
...
mod integration_tests {
    ...
    async fn test_file_entry_calculated_size_updated() {
        ...
        assert_eq!(
            dir_entry.calculated_size, None,
            "Initially calculated_size should be None"
        );
